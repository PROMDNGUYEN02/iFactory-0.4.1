...
Enhanced with complete semantic token support and modular QSS.
"""
...
class ThemeTokens:
    ...
    - Interactive: hover, active, selected states
    - Semantic: primary, success, warning, error, info
    - Status: machine status colors
    - Chart: data visualization colors
    - Layout: sizing constants

    Also includes legacy aliases for backward compatibility.
    """
    ...
    @property
    def app_bg(self) -> str:
        """Legacy alias for surface_app."""
        return self.surface_app

    @property
    def app_fg(self) -> str:
        """Legacy alias for text_primary."""
        return self.text_primary

    @property
    def slide_bg(self) -> str:
        """Legacy alias for surface_panel."""
    ...
    def frame_bg(self) -> str:
        """Legacy alias for surface_card."""
        return self.surface_card

    @property
    def border(self) -> str:
        """Legacy alias for border_default."""
        return self.border_default

    @property
    def hover(self) -> str:
        """Legacy alias for interactive_hover."""
        return self.interactive_hover

    @property
    def selected(self) -> str:
        """Legacy alias for interactive_selected_bg."""
        return self.interactive_selected_bg

    @property
    def selected_text(self) -> str:
        """Legacy alias for interactive_selected_text."""
...
class ThemeService(QObject):
    ...
    Enhanced features:
    - Complete semantic token support
    - Modular QSS loading
    ...
    def tokens(self) -> ThemeTokens:
        """Get semantic color tokens for current theme."""
    ...
    def _get_merged_variables(self) -> Dict[str, str]:
        """Merge common and theme-specific variables, excluding comments."""
        common = {k: v for k, v in self._variables.get("common", {}).items() if not k.startswith("__")}
        theme_vars = {k: v for k, v in self._variables.get(self._current_theme, {}).items() if not k.startswith("__")}
    ...
    def get_stylesheet(self) -> str:
        """Get compiled stylesheet for current theme."""
        cache_key = self._current_theme

        if cache_key in self._stylesheet_cache:
            return self._stylesheet_cache[cache_key]

        # Try modular loading first, fall back to legacy
        if self._styles_path.exists():
            stylesheet = self._load_modular_stylesheet()
        else:
            stylesheet = self._compile_legacy_stylesheet()
    ...
    def _compile_legacy_stylesheet(self) -> str:
        """Compile legacy base.qss template."""
    ...
    def get_input_style(self) -> str:
        """Get pre-computed input field style."""
        tokens = self.tokens
    ...
    def get_scrollbar_style(self) -> str:
        """Get pre-computed scrollbar style."""
        tokens = self.tokens
    ...
    def get_tooltip_style(self) -> str:
        """Get pre-computed tooltip style."""
        tokens = self.tokens
    ...
    def get_list_widget_style(self) -> str:
        """Get pre-computed list widget style."""
        tokens = self.tokens
