...
class SQLAlchemyMapper:
    ...
    def to_status_period(model: Optional[StatusHistoryModel]) -> Optional[StatusPeriod]:
        ...
        return StatusPeriod(
            equipment_code=EquipmentCode(model.equip_code),
            status=status,
            time_range=TimeRange(model.start_time, effective_end),
        )

    @staticmethod
    def to_status_period_model(
        vo: StatusPeriod,
    ...
    def to_material_input(
        model: MaterialInputHistoryModel | LatestMaterialInputModel | None,
    ...
    def to_material_history_model(vo: MaterialInput) -> MaterialInputHistoryModel:
        return MaterialInputHistoryModel(
            id=str(uuid4()),
            equipment_code=vo.equipment_code.value,
            material_batch=vo.material_batch.value,
            feeding_time=vo.feeding_time,
