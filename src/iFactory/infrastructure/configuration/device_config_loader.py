...
def _get_status_from_code(code: Any) -> str:
    return STATUS_CODE_MAP.get(str(code), "unknown")


def _get_status_color(status: str, theme: str = "light") -> str:
    theme_colors = STATUS_COLORS.get(theme, STATUS_COLORS["light"])
    return theme_colors.get(status.lower(), theme_colors["unknown"])


def _get_status_display(status: Any) -> str:
    """Get display name for status (handles both code and name)."""
    status_str = str(status)
    return STATUS_DISPLAY.get(status_str, STATUS_DISPLAY.get(status_str.lower(), status_str.upper()))
...
class DeviceData:
    ...
    def set_status_from_code(self, code: Any) -> None:
        ...
        self.status = _get_status_from_code(code)

    def get_status_color(self) -> QColor:
        hex_color = _get_status_color(self.status, self.theme)
...
class DeviceSvgWidget(QWidget):
    ...
    def _show_tooltip(self) -> None:
            ...
            try:
                ...
                if data:
                    ...
                    if db_status_code is not None:
                        ...
                        self._schedule_update()
                    if db_status_code is not None:
                        display = db_status_name.upper() if db_status_name else _get_status_display(db_status_code)
        ...
        canvas_display = _get_status_display(device.status)
...
class DeviceConfigLoader(QObject):
    ...
    def update_all_status(self, status_dict: Dict[str, Any]) -> None:
        ...
        Supports both legacy (Dict[str, str]) and new (Dict[str, Dict]) formats
        for backward compatibility.

        Args:
            status_dict: Either:
                - Dict[str, str]: device_id -> status_code
                - Dict[str, Dict]: device_id -> full status dict
        """
        updated = 0
